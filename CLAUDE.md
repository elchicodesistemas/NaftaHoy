...
- **Seguridad de `/api/*` (2026-08-05):** `backend/src/middleware/apiKey.ts` exige header `x-api-key` == `API_KEY` de `.env` (401 si falta o no matchea). `backend/src/middleware/rateLimit.ts` (paquete `hono-rate-limiter`) limita a 100 req/15min, en memoria del proceso — si se escala a más de una instancia del server, esto necesita un store compartido (Redis). Orden en `index.ts`: rate limit primero (por API key o IP), auth después. `/health` queda afuera de las dos cosas a propósito.
- Colección de Postman para probar todo esto: `backend/postman/NaftaHoy-API.postman_collection.json` (importar y completar la variable `apiKey` a mano, nunca commitear el valor real ahí).
