...
export const estaciones = pgTable(
  ...
  (table) => [
    unique("estaciones_fuente_external_id_unique").on(
      table.fuente,
      table.externalId,
    ),
    index("estaciones_provincia_localidad_idx").on(
      table.provincia,
      table.localidad,
    ),
...
export const preciosActuales = pgTable(
  "precios_actuales",
  {
    ...
    fechaCapturado: timestamp("fecha_capturado", { withTimezone: true })
      .notNull()
      .defaultNow(),
    fuente: text("fuente").notNull(),
  },
  (table) => [
    unique("precios_actuales_estacion_combustible_unique").on(
      table.estacionId,
...
export const preciosHistorico = pgTable(
  ...
  (table) => [
    ...
    index("precios_historico_tendencia_idx").on(
      table.tipoCombustible,
      table.fechaVigencia,
    ),
  ],
...
export const reportesPrecio = pgTable("reportes_precio", {
  ...
  petroleraId: uuid("petrolera_id")
    .notNull()
    .references(() => petroleras.id, { onDelete: "restrict" }),
  estacionId: uuid("estacion_id").references(() => estaciones.id, {
    onDelete: "set null",
  }),
  tipoCombustible: text("tipo_combustible").notNull(),
  region: text("region").notNull(), // texto libre, ej. "CABA - Palermo"
  precioReportado: numeric("precio_reportado", {
    precision: 10,
    scale: 2,
  }).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true })
